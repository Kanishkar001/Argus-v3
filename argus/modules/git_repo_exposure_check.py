...
def run(target, threads, opts):
    ...
    # ── Build table (FIXED: now correctly inside run()) ──────────────────────
    tbl = Table(title=f".git Exposure – {dom}", header_style="bold white")
    for h in ("Path", "Status", "Bytes", "Exposed?"):
    ...
    if EXPORT_SETTINGS["enable_txt_export"]:
        ...
        # ── FIXED: render Rich table to string via StringIO ───────────────────
        _buffer = StringIO()
        _con    = Console(file=_buffer, no_color=True)
        _con.print(tbl)
        write_to_file(os.path.join(out, "git_exposure.txt"), _buffer.getvalue())
